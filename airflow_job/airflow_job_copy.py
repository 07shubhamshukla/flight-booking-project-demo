...
with DAG(
    ...
    env = Variable.get("env", default_var="dev") # this will get environment variable from airflow UI and if not present then default_var is dev
    gcs_bucket = Variable.get("gcs_bucket", default_var="airflow-projects-gds-dev-2")
    bq_project = Variable.get("bq_project", default_var="project-6-08-2026-504717")
    bq_dataset = Variable.get("bq_dataset", default_var=f"flight_data_{env}") # env will automatically fetch data from airflow environment. if airflow run job on dev it will fetch or prod
    ...
    # this particular task is continuously in running state.
    ...
    batch_details = {
        "pyspark_batch": {
            "main_python_file_uri": f"gs://{gcs_bucket}/flight-booking-analysis/spark-job/spark_transformation_job_copy.py",  # Main Python file
        ...
        "environment_config": {
            "execution_config": {
                "service_account": "905475531274-compute@developer.gserviceaccount.com", # Get details from I am admin
